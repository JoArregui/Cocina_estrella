...
class IngredientScanScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<IngredientScanScreen> createState() =>
      _IngredientScanScreenState();
}

class _IngredientScanScreenState extends ConsumerState<IngredientScanScreen>
  ...
  Future<void> _analyze() async {
    if (_inputMode == 'camera' && _image != null) {
      await ref
          .read(chefIaProvider.notifier)
          .analyzeImage(ImageFileWrapper(_image!));
    } else if (_inputMode == 'text' && _textController.text.trim().isNotEmpty) {
      await ref
          .read(chefIaProvider.notifier)
          .analyzeText(_textController.text.trim());
    }
  }
        ...
        child: Column(
          ...
          children: [
            ...
            if (state.ingredients.isNotEmpty) ...[
              ...
              _buildIngredientsFound(state.ingredients),
            ],
  ...
  Widget _buildInputToggle() => Container(
  ...
  Widget _buildTextInput() => Container(
    ...
    child: TextField(
      ...
      decoration: InputDecoration(
        hintText: 'Ej: pollo, tomate, cebolla, pimiento, arroz...',
  ...
  Widget _buildCameraInput() => Column(
    children: [
      _image != null
          ? Stack(
              children: [
          ...
          : GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(16),
                  border: Border.all(
                    color: const Color(0xFFE8490F).withValues(alpha: 0.3),
                    width: 2,
                  ),
                  boxShadow: [
                    BoxShadow(
  ...
  Widget _buildAnalyzeButton(ChefIaState state) {
    ...
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        onPressed: canAnalyze && !state.analyzing ? _analyze : null,
        icon: const Icon(Icons.auto_awesome, size: 20),
        label: Text(
          state.analyzing ? 'Analizando...' : 'Sugerir platos con IA',
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  isKeyError
                      ? 'API Key inválida (${ai.providerName})'
  ...
  Widget _buildSuggestions(ChefIaState state) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Row(
        children: [
          ...
          const Spacer(),
          Text(
            '${state.suggestions.length} opciones',
            style: GoogleFonts.nunito(
              fontSize: 13,
              color: const Color(0xFF888888),
            ),
      ...
      ...state.suggestions.asMap().entries.map((entry) {
        ...
        final s = entry.value;
        final meal = index < state.matchedMeals.length
            ? state.matchedMeals[index]
            : null;
  ...
  void _openCookingMode(
    ...
    final fakeMeal = Meal(
      id: mealSummary?.id ?? '',
      name: suggestion.nameEs,
      category: suggestion.difficulty,
      area: suggestion.time,
      instructions: '',
      thumbnail: mealSummary?.thumbnail ?? '',
      ingredients: ingredients
          .map((ing) => MealIngredient(name: ing, measure: ''))
          .toList(),
    );
    final steps = [
      'Prepara y lava todos los ingredientes: ${ingredients.join(', ')}.',
      'Corta y trocea los ingredientes según sea necesario.',
      'Calienta la sartén o cacerola a fuego medio con un poco de aceite.',
      'Añade los ingredientes en orden según la receta de ${suggestion.nameEs}.',
      'Cocina durante aproximadamente ${suggestion.time} hasta que esté listo.',
      'Sazona al gusto con sal, pimienta y las especias que prefieras.',
      '¡Sirve caliente y disfruta tu ${suggestion.nameEs}!',
    ];
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => CookingModeScreen(meal: fakeMeal, steps: steps),
      ),
    );
  }
}
